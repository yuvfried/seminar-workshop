...
def fit_decay_to_signal(df_model, halflife, regularization_vals, auc_noise):
    ...
    # fit regression for each lambda
...
def fit_h_to_signal(df_model, halflife_vals, auc_noice):
    y = simulate_auc(df_model["S"], sigma=auc_noice).iloc[1:]   # we need t-1 for making explained variables
    best_r2 = -np.inf
    best_h = None
    
    for h in halflife_vals:
        features = get_regression_features(df_model, h)
        reg = LinearRegression()
        reg.fit(features, y)
        r2_score = reg.score(features, y)
        if r2_score > best_r2:
            best_r2 = r2_score
            best_h = h
...
def convert_N_to_h(model_surprise_dict, halflife_vals, auc_noise):
    d_results = dict()
    for n in tqdm(Ns, desc='N'):
        d_results[n] = dict()
        for beta_ind in tqdm(range(len(betas)), desc=BETA_STR, leave=False):
            df_model = model_surprise_dict[n, beta_ind].dropna()
            h, r2 = fit_h_to_signal(df_model, halflife_vals, auc_noise)
            d_results[n][beta_ind] = {'h':h, 'r2':r2}
